...
const ELD = () => {
    ...
    const records = useSelector(state => state.auth)
    const {elddevices} = useSelector(state => state.elddevice)
    const totalRecords = elddevices.TotalRecord
    console.log(elddevices)
    useEffect(()=>{
    ...
    const submitHandler = (e) => {
        ...
        if (searchKey !== "" && searchStatus !== "") {
          dispatch(getELDDevices(searchKey,searchStatus));
        }
                ...
                <div className="col-12">
                  <div className="table-responsive mb-0">
                    <table align="left"  className="table table-background   dt-responsive " style={{borderCollapse: "collapse", borderSpacing: "0", width: "100%"}}>
                      ...
                      <tbody>
                        {elddevices?.elds && elddevices?.elds.map((eld,idx) => (
                            <tr key={idx}>
